...
!function ( $ ) { 
	...
	function InfiniteScroll( element, options ) {
		...
		if( ! this.$el.data('inifinitescroll' ))
		{
			...
			}, $.fn.inifinitescroll.defaults, options );
	...
	InfiniteScroll.prototype = {
		...
		init: function() {
			...
			this._url = this.$el.attr('data-href');
...
//			this.$el.hide();
		...
		url: function(){
			return this._url + (this._url.indexOf("?")>-1?'&':'?') + this.query.replace('{pageNumber}', this.pageNumber).replace("{pageSize}",this.pageSize);
		},
